...
def _get_connection() -> duckdb.DuckDBPyConnection:
    """Create a DuckDB connection with HuggingFace auth."""
    con = duckdb.connect()
...
def load_disorder(disorder: DisorderDataset, p_threshold: float = 1e-5) -> pl.DataFrame:
    ...
    for canonical, source in col_map.items():
        if canonical in ("snp", "chr", "bp", "p", "beta", "or_val", "se", "freq"):
            # Quote column names to handle reserved words like OR
    ...
    # Query with predicate pushdown — only downloads matching rows
    query = f"""
        SELECT {select_str}
        FROM '{hf_path}'
